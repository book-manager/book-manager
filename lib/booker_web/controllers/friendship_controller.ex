...
defmodule BookerWeb.FriendshipController do
  ...
  def create(conn, %{"friend_id" => friend_id}) do
    ...
    query = from f in Booker.Auth.Friendship,
    ...
    case response do
      [] ->
        with {:ok, %Friendship{} = friendship} <- Auth.create_friendship(%{friend_a_id: current_user.id, friend_b_id: friend_id, pending: true}) do
          conn
          |> put_status(:created)
  ...
  def check_friendship(conn, %{"friend_id" => friend_id}) do
    ...
    outgoing_query = from u in Booker.Auth.Friendship,
    ...
    incoming_query = from u in Booker.Auth.Friendship,
    ...
    case response do
      [] ->
        case incoming_response do
          [] ->
            Logger.info("Friendship between #{current_user_id} and #{friend_id} is in state: false")
            conn
            |> render("check-friendship.json", friendship: false)
          [%Friendship{pending: true}] ->
            Logger.info("Friendship between #{current_user_id} and #{friend_id} is coming from other user")
            conn
